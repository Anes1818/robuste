...
function renderProducts(products) {
    ...
    for (var i = 0; i < products.length; i++) {
        ...
        var discountBadge = product.old_price && product.old_price > product.price ? 
            '<div class="discount-badge">' +
                'خصم ' + Math.round(((product.old_price - product.price) / product.old_price) * 100) + '%' +
            '</div>' : '';
        
        var oldPrice = product.old_price && product.old_price > product.price ? 
        ...
        for (var k = 0; k < product.images.length; k++) {
            carouselItems += '<div class="carousel-item ' + (k === 0 ? 'active' : '') + '">' +
                '<img src="' + product.images[k] + '" class="d-block w-100" alt="' + product.title + '" loading="lazy">' +
            '</div>';
        }
...
// Touch state tracking for iOS Safari
var touchState = {
    startX: 0,
    startY: 0,
    startTime: 0,
...
// Single global touchstart handler for product cards
document.addEventListener('touchstart', function(e) {
    ...
    var interactiveEl = e.target.closest('button, a, input, select, textarea, .carousel-control, .carousel-indicators, .add-to-cart-btn');
    if (interactiveEl) {
        // For add-to-cart buttons, handle separately
        if (interactiveEl.classList.contains('add-to-cart-btn')) {
            e.stopPropagation();
            return;
        }
        return;
...
// Single global touchmove handler for swipe detection
document.addEventListener('touchmove', function(e) {
    if (!touchState.touchStarted || !touchState.targetCard) return;
    
    var touch = e.touches[0];
    var deltaX = Math.abs(touch.clientX - touchState.startX);
    var deltaY = Math.abs(touch.clientY - touchState.startY);
    
    // If movement is significant, treat as scroll/swipe
    if (deltaX > 10 || deltaY > 10) {
        touchState.isScrolling = true;
        touchState.targetCard.classList.remove('touch-active');
    }
}, { passive: true });

// Single global touchend handler for tap detection
document.addEventListener('touchend', function(e) {
    if (!touchState.touchStarted || !touchState.targetCard) {
        touchState.touchStarted = false;
        return;
    }
    
    var card = touchState.targetCard;
    var elapsedTime = Date.now() - touchState.startTime;
    
    // Remove visual feedback
    card.classList.remove('touch-active');
    
    // Check if it was a valid tap (not scroll, not too long)
    if (!touchState.isScrolling && elapsedTime < 300) {
        // Check if we're still on the same card
        var endEl = e.changedTouches[0];
        var endCard = document.elementFromPoint(endEl.clientX, endEl.clientY);
        if (!endCard || !endCard.closest || !endCard.closest('.product-card')) {
            touchState.touchStarted = false;
            return;
        }
        
        // Check if tap ended on interactive element
        var interactiveEl = endCard.closest('button, a, input, select, textarea, .carousel-control, .carousel-indicators, .add-to-cart-btn');
        if (interactiveEl) {
            touchState.touchStarted = false;
            return;
        }
        
        // Get product ID and navigate
        var pid = card.getAttribute('data-pid') || card.querySelector('.add-to-cart-btn').getAttribute('data-id');
        if (pid) {
            // Use setTimeout to allow browser to process click if needed
            setTimeout(function() {
    ...
    // Reset touch state
    touchState.touchStarted = false;
    touchState.targetCard = null;
}, { passive: true });

// Add touch-cancel handler
document.addEventListener('touchcancel', function() {
    if (touchState.targetCard) {
        touchState.targetCard.classList.remove('touch-active');
    }
    touchState.touchStarted = false;
    touchState.targetCard = null;
...
// Keep existing click handler for non-touch devices
document.addEventListener('click', function(e) {
    // Check if this is a touch device (iOS Safari reports touches)
    if ('ontouchstart' in window && e.touches) {
        // For touch devices, we rely on touch handlers
        return;
    }
    ...
    // If click came from interactive element, ignore
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        var pid = focused.getAttribute('data-pid') || focused.querySelector('.add-to-cart-btn').getAttribute('data-id');
        if (pid) window.location.href = 'product.html?pid=' + encodeURIComponent(pid);
    }
});

// ============== ADD TO CART BUTTON HANDLING (iOS OPTIMIZED) ==============
function setupAddToCartButtons() {
    if (setupAddToCartButtons._initialized) return;
    setupAddToCartButtons._initialized = true;
    
    // Single global handler for add-to-cart buttons
    document.addEventListener('click', function(e) {
    ...
    // Touch handler specifically for iOS Safari
    document.addEventListener('touchend', function(e) {
        var button = e.target.closest('.add-to-cart-btn');
        if (!button) return;
        
        // Only handle if this is the primary button touch
        e.stopPropagation();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add touch-action CSS for better iOS performance
    var style = document.createElement('style');
    style.textContent = '.product-card { touch-action: manipulation; }';
